...
| Madde                      | Not                                                                                            |
| -------------------------- | ---------------------------------------------------------------------------------------------- |
| PathUtils-safe helpers     | `safeFilename` / `safePathKey` / `safeParent` — boş, göreli, `attachments:` yollarda throw yok |
| İki uçlu Tara dayanıklılık | eşleşme+hash try/catch; `writeBidirReport` last- fallback; openLast stamped report fallback    |
| Raporu aç kök neden        | 1.0.153’te Tara PathUtils’te çöküyordu → `last-bidirectional.json` yoktu                       |
...
| Madde                  | Not                                                                                  |
| ---------------------- | ------------------------------------------------------------------------------------ |
| Disk / ek denetimi Çöz | Tara → Rapor → Çöz (orphan ID+karantina, rename, kopya karantina); otomasyon ayrı h3 |

### Son eklenen (v1.0.148)

| Madde          | Not                                                    |
| -------------- | ------------------------------------------------------ |
| `tr-TR` locale | Menü + tercihler öğretici Türkçe; en-US anahtar parity |

### Son eklenen (v1.0.147)

| Madde                        | Not                                                                                      |
| ---------------------------- | ---------------------------------------------------------------------------------------- |
| autoAttach/review eşik float | `ensureUnitIntervalPref` — prefs açılınca 0.85/0.6→0 floor bug’ı; coverage demote mesajı |
