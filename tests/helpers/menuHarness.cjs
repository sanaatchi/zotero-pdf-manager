...
const PREF_PREFIX = "extensions.zotero.zpdfmanager";
...
function createHarness(options = {}) {
  ...
  global.IOUtils = {
    ...
    getChildren: async (dir) => {
      const normalizedDir = path.posix.normalize(dir);
      return [...directories, ...files]
        .map((value) => path.posix.normalize(value))
        .filter(
          (value) =>
            value !== normalizedDir &&
            path.posix.dirname(value) === normalizedDir,
        );
    },
    stat: async (value) => ({
      type: directories.has(path.posix.normalize(value))
        ? "directory"
        : "regular",
    }),
  ...
  global.ZoteroPane = {
    ...
    selectItem: async (id) => calls.selectItem.push(id),
    viewAttachment: async () => {},
  };
  global.Zotero = {
    Attachments: {
      getFileBaseNameFromItem: (item, renameOptions = {}) => {
        if (options.getFileBaseNameFromItem) {
          return options.getFileBaseNameFromItem(item, renameOptions);
        }
        return options.baseName || item.fileBaseName || "renamed";
      },
    ...
    CreatorTypes: {
      getID: (name) => ({ author: 1, editor: 2, seriesEditor: 3 })[name],
    ...
    File: {
      ...
      iterateDirectory: async (dir, callback) => {
        const normalizedDir = path.posix.normalize(dir);
        ...
        for (const directory of directories) {
          ...
          if (
            ...
            path.posix.dirname(normalized) === normalizedDir
          ) {
        ...
        for (const file of files) {
          ...
          if (path.posix.dirname(normalized) === normalizedDir) {
            children.set(normalized, {
              isDir: false,
              name: path.posix.basename(normalized),
              path: normalized,
